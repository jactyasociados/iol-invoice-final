...
def upload():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            metadata, res = dbx.sharing_get_shared_link_file(url=result)
            #name_url=result.replace("https:","")
            #name_url_final=name_url.replace("?dl=0","")
            print(metadata.url)
            #print(res.content)  
            #name_url=metadata.url.replace("https:","http:")
    ...
    try:
        if request.method == 'POST':
           ...
           @frame footer_frame {           /* Another static Frame */ \
            ...
            f.write("</table><div><pdf:nextpage></div>")
            query.next
            f.close()
            '''f=open("uploads/" + name + ".html","a")
